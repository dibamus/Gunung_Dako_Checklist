...
accCurve <- function(df, palette){
  ...
  band1 <- levels(df$eband)[1]
  band2 <- levels(df$eband)[2]
  band3 <- levels(df$eband)[3]
  
  #figure out what dyaes & days to use for plotting & organization
  ...
  bandNewSp <- cbind(day = banddays, #adding 1 day so NA replacements below work
                     b1 = sapply(banddays, countSpEl, df = df, el = band1)%>%cumsum,
                     b2 = sapply(banddays, countSpEl, df = df, el = band2)%>%cumsum,
                     b3 = sapply(banddays, countSpEl, df = df, el = band3)%>%cumsum) %>%
    as.data.frame
  
  #Remove points after the end of band sampling
  bandNewSp[which(bandNewSp$day > max(na.omit(filter(df, eband == band1)$dayinband))),]$b1 <- NA
  bandNewSp[which(bandNewSp$day > max(na.omit(filter(df, eband == band2)$dayinband))),]$b2 <- NA
  bandNewSp[which(bandNewSp$day > max(na.omit(filter(df, eband == band3)$dayinband))),]$b3 <- NA
  ...
  bandAccTable <- melt(bandNewSp, id.vars = 'day') %>% rename(c('day' = 'day',
    ...
    mutate(band = recode(band, b1 = band1, b2 = band2, b3 = band3)) %>%
...
elevBands <- function(df,b1max,b2max,absmax = 3000){
  ...
  #set up elevation band labels
  band1 <- paste0("<",b1max,"m")
  band2 <- paste0(b1max, "-", b2max, "m")
  band3 <- paste0(">",b2max,"m")
  
  #assign bands to specimens
  df$eband <- cut(df$Elevation, 
                  breaks = c(0,b1max, b2max, absmax),
                  labels = c(band1, band2, band3)) %>%
    as.factor()
  
  df$dayinband <- 0
  
  # Label by Day within elevational band 1
  map1 <- setNames(c(1:length(unique(df[which(df$eband == band1),]$Collection_Date))),
                   unique(df[which(df$eband == band1),]$Collection_Date))
  df[which(df$eband == band1),]$dayinband <- map1[as.character(df[which(df$eband == band1),]$Collection_Date)]
  
  # Label by Day within elevational band 2
  map2 <- setNames(c(1:length(unique(df[which(df$eband == band2),]$Collection_Date))),
                   unique(df[which(df$eband == band2),]$Collection_Date))
  df[which(df$eband == band2),]$dayinband <- map2[as.character(df[which(df$eband == band2),]$Collection_Date)]
  
  # Label by Day within elevational band 3
  map3 <- setNames(c(1:length(unique(df[which(df$eband == band3),]$Collection_Date))),
                   unique(df[which(df$eband == band3),]$Collection_Date))
  df[which(df$eband == band3),]$dayinband <- map3[as.character(df[which(df$eband == band3),]$Collection_Date)]
  ...
  #I know it's ghastly
  #band 1
  df$firstinband[which(df$eband == band1)[match(unique(df$Genus_species[which(df$eband == band1)]),
                                                df$Genus_species[which(df$eband == band1)])]] <- TRUE
  #band 2
  df$firstinband[which(df$eband == band2)[match(unique(df$Genus_species[which(df$eband == band2)]),
                                                df$Genus_species[which(df$eband == band2)])]] <- TRUE
  #band 3
  df$firstinband[which(df$eband == band3)[match(unique(df$Genus_species[which(df$eband == band3)]),
                                                df$Genus_species[which(df$eband == band3)])]] <- TRUE
...
missingSpecies <- function(df) {
  ...
  diversityEstimates <- rbind(specpool(veganize(df), smallsample = TRUE),
                              specpool(veganize(filter(df, eband == levels(df$eband)[1])), smallsample = TRUE),
                              specpool(veganize(filter(df, eband == levels(df$eband)[2])), smallsample = TRUE),
                              specpool(veganize(filter(df, eband == levels(df$eband)[3])), smallsample = TRUE),
  ...
  rownames(diversityEstimates) <- c("Total Mountain",
                                    paste("Low elevation ",levels(df$eband)[1]),
                                    paste("Middle elevation ",levels(df$eband)[2]),
                                    paste("High elevation ",levels(df$eband)[3]),
                                    "Frogs",
                                    "Lizards",
                                    "Snakes")
...
plotElev <- function(df, colors){
  ...
  b1max <-as.numeric(regmatches(levels(edf$eband)[2],(gregexpr("[0-9]+",levels(edf$eband)[2])))[[1]][1])
  b2max <-as.numeric(regmatches(levels(edf$eband)[2],(gregexpr("[0-9]+",levels(edf$eband)[2])))[[1]][2])
  ...
  scatter <- ggplot() +
    geom_hline(yintercept = b1max, color = colors[length(colors)], linetype = "dashed") +
    geom_hline(yintercept = b2max, color = colors[length(colors)], linetype = "dashed") +
